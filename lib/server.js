...
let messageQueue = []; //Keep last 100 messages
...
*/
  ...
  socket.on('user_logout', (uname) => {
    ...
    console.log(connectedUsersNames);
  });
/*
*     CHAT STUF
*
*/
  ...
  socket.on('mes', (userName, mes) => {
    ...
    let mesLog = `{"dateTime" : "${dateTime.toDateString()}", "userName" : "${userName}", "message" : "${mes}"}`;
    messageQueue.push(mesLog);
    if(messageQueue.length == 101) {
      messageQueue.shift();
    }
