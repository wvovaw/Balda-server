...
function findInitWord() {
  let i = Math.floor(Math.random() * inits.length - 1);
  return(inits[i]);
}
...
*/
  ...
  socket.on('player_leave_lobby', (playerName, lobbyId) => {
    ...
    for(l of lobbyList) {
      if(l.id == lobbyId) {
        l.kickPlayer(playerName, socket, io);
        break;
      }
    }
  });
  ...
  socket.on('check_word', (word) => {
    if(dict.includes(word.toLowerCase())) {
      ...
      socket.emit('check_word_result', true);
    }
    else {
      console.log(`The word '${word}' does not exist.`);
      socket.emit('check_word_result', false);
    }
  });
  ...
  socket.on('i_skip_turn', (lobbyId, playerName) => {
    for(l of lobbyList) {
      if(l.id == lobbyId) {
        for(p of l.playerList) {
          if(p.playerName == playerName) {
            p.addPenalty();
            if(p.penalty == 3) {
              io.to(lobbyId).emit('looser', playerName);
              l.kickPlayer(playerName, socket, io);
            }
            io.to(lobbyId).emit('board_changes', null, playerName, null, null);
            l.nextTurn(io, true);
            return;
          }
        }
...
class Lobby {
  ...
  addPlayer(userName, userAvatar, socket) {
    if(this.playerList.length < this.maxPlayers) {
      ...
      io.emit('lobbyListChanges');
      socket.join(this.id);
      socket.to(this.id).emit('playerConnected', player.playerName, player.playerAvatar, player.getPoints());
    }
  }
  kickPlayer(playerName, socket, io) {
    //console.log('BEFORE:');
    //console.log(this.playerList);
    for(p of this.playerList) {
      if(p.playerName == playerName) {
        this.playerList.splice(this.playerList.indexOf(p), 1);
        socket.leave(this.id);
        console.log(`Player '${playerName}' disconnected from the '${this.title}'`);
        io.to(this.id).emit('playerDisconnected', playerName);
        if(this.playerList.length == 1 && this.gameHasBegun == true) {
          this.endGame(io);
        }
  ...
  nextTurn(io, isSkiped = false) {
    if(this.turnCounter == this.endOfTheGameTurn || this.playerList.length < 2) {
      this.endGame(io);
    }
    else {
      if(this.nowTurnPlayer == this.playerList.length - 1)
        this.nowTurnPlayer = 0;
      else this.nowTurnPlayer = this.nowTurnPlayer + 1;
      if(isSkiped == false) this.turnCounter++;
      io.to(this.id).emit('now_turns', this.playerList[this.nowTurnPlayer].playerName);
